...
class MarketUserHistory {
    ...
    void addProductToCartAndBuyIt() throws Exception {
        ...
        UserInvoice userInvoice = market.getUserPurchaseHistoryByAdmin(admin, cNames[0]).stream().toList().get(0);
        for (ShopInfo shopInfo : userInvoice.getProductInfoInShop().values()){
            ProductInfo productInfo = shopInfo.getProductInfos().get(0);
    ...
    void purchaseMoreThanProductQuantityInShop() throws Exception {
        try{
            ...
            Collection<UserInvoice> userInvoices = market.getUserPurchaseHistoryByAdmin(admin, cNames[0]);
            assertTrue(userInvoices == null || userInvoices.size() == 0);
        }
    }
